...
#define CAN_NODE_CONFIGURATOR_CHANNEL			CAN_NODE1		// Select the CAN_Node number chosen in the MTB Device Configurator; 			CAN Node 0 ... CAN_NODE0		CAN Node 1 ... CAN_NODE1

/* IRQ Event Source Names for XMC4700 - see XMC4700 Reference Manual Table 5-1 */
#define CAN_IRQ_RX_NUMBER               		IRQ3_IRQn         /* CAN Interrupt Number Setting		-	  XMC1404 = IRQ3_IRQn; 		XMC4700 = CAN0_0_IRQn */
#define CAN_IRQ_RX_MESSAGE_HANDLER         		IRQ3_Handler  		/* CAN Interrupt Handler Setting	-	  XMC1404 = IRQ3_Handler;	XMC4700 = IRQ_Hdlr_76 */
...
/* This library is designed to work with the IMR CAN identification in mind; oards are identified using DIP switches according to the selected board type;
 ...
 * 		 	Set the desired Timer Frequency (Prescaler Initial Value - 2048 and Timer Period Value - 5860 for 4Hz)
 * 		 	Activate "Period Match while Counting Up" and set "Period Match Up Service Request" to Service Request 2
 * 		 	Deactivate "Start After Initialization"
 *
 * 		 Activate Slice 0 for CCU4_1
 * 		 	Set the desired Timer Frequency (Prescaler Initial Value - 64 and Timer Period Value - 15000 for 100Hz)
 * 		 	Activate "Period Match while Counting Up" and set "Period Match Up Service Request" to Service Request 0
 * 		 	Deactivate "Start After Initialization"
 * */
...
#define CAN_NODE_RX_TIMEOUT_CCU4_NAME			CAN_TIMEOUT		// Select the CAN_Node RX timeout timer name chosen in the MTB Device Configurator (CCU4);
#define CAN_NODE_TX_TIMER_CCU4_NAME				CAN_TX_TIMER	// Select the CAN_Node TX timer name chosen in the MTB Device Configurator (CCU4);
...
#define CAN_NODE_RECEIVE_LED_ENABLE				(1U)			// Select if a specific LED should indicate the receive of a CAN message; 		0U ... CAN RX LED NOT used; 		1U ... CAN RX LED used;
...
/* IRQ Event Source Names for XMC1404 - see XMC1400 Reference Manual Table 5-1 */
#define IRQ_NUMBER_CAN_TIMEOUT_TIMER 					IRQ16_IRQn			/* Defines IRQ number of the period match event interrupt */
#define IRQ_NUMBER_CAN_TIMEOUT_TIMER_HANDLER 			IRQ16_Handler		/* Defines handler of the period match event interrupt */

#define IRQ_NUMBER_CAN_TX_TIMER 						IRQ21_IRQn			/* Defines IRQ number of the period match event interrupt */
#define IRQ_NUMBER_CAN_TX_TIMER_HANDLER 				IRQ21_Handler		/* Defines handler of the period match event interrupt */
