...
void SPI_master_init(XMC_USIC_CH_t * const channel) {
  ...
  XMC_SPI_CH_ConfigureShiftClockOutput(channel, XMC_SPI_CH_BRG_SHIFT_CLOCK_PASSIVE_LEVEL_0_DELAY_DISABLED, XMC_SPI_CH_BRG_SHIFT_CLOCK_OUTPUT_SCLK); //Falling sclk sampling and no polarity inversion
...
{
  uint32_t systickCount = SystemVar.GlobalTimer;
  ...
  {
	  if (SystemVar.GlobalTimer > systickCount)
	  { error_SPI = 1;
	        break;
	  }
  };
